...
通过 [foxapi.cc](https://docs.foxapi.cc/pages/zh/api-manual/image-series/gpt-image-2/gpt-image-2) 的 `gpt-image-2` 接口创建异步图片生成任务，创建成功后必须继续查询直到任务终态（`completed` 或 `failed`）。鉴权采用 `Authorization: Bearer <key>`。
...
环境变量名 `X_API_KEY` 是历史延续命名；值通过 `Authorization: Bearer <key>` 提交给 foxapi.cc。
...
# 图生图（多张参考图重复传 --image-url，最多按 foxapi.cc 实际上限）
X_API_KEY='sk-xxx' ./scripts/create_task.sh \
  --prompt "基于参考图重绘成极简海报" \
